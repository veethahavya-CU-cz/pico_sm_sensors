...
if file_exists(config_path):
    ...
    log_path = CONFIG['fpath']['log']
    log_level = CONFIG['IO']['log']['level']
    if CONFIG['IO']['log']['UART']:
        try:
            uart_obj = UART(CONFIG['Pin']['UART']['BUS'], baudrate=CONFIG['time']['interval']['UART_BAUD'], tx=CONFIG['Pin']['UART']['TX'], rx=CONFIG['Pin']['UART']['RX'])
        except KeyError:
            uart_obj = None
else:
...
def move(new_log_path, rewrite=False):
    global log_path
